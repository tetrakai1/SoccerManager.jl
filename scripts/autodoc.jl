...
options = Dict(
    :min_args => 1, # Minimum number of arguments to print the argument list
    :args_header => "# Arguments", # Printed above the argument list
    :kwargs_header => "# Kwargs", # Printed above the keyword argument list
    :struct_fields_header => "# Fields", # Printed above the fields list
    :arg_types_in_desc => true, # Include the argument types in the description
...
function build_function_doc_string(mp::Expr)
    ...
    if length(fundef[:args]) + length(fundef[:kwargs]) >= options[:min_args]
        ...
        if !isempty(fundef[:args])
            ...
            maxchar  = maximum(length.(get_arg_item_str.(fundef[:args], options[:arg_types_in_desc], 
                                                         options[:defaults_in_desc])))
            argnames = [MacroTools.splitarg(fundef[:args][i])[1] for i in eachindex(fundef[:args])]
            maxchar2 = maximum(length.(string.(argnames)))

            str = str * join(map(fundef[:args]) do f
                "- `" * get_arg_item_str(f, options[:arg_types_in_desc], 
                                         options[:defaults_in_desc], maxchar, maxchar2) * 
                "`" * " : DESCRIPTION" end, "\n")
        end
        str = str * "\n"
        if !isempty(fundef[:kwargs])
            !isnothing(options[:kwargs_header]) && (str = str * "\n" * "$(options[:kwargs_header])\n")

            maxchar  = maximum(length.(get_arg_item_str.(fundef[:kwargs], options[:arg_types_in_desc], 
                                                         options[:defaults_in_desc])))
            kwargnames = [MacroTools.splitarg(fundef[:kwargs][i])[1] for i in eachindex(fundef[:kwargs])]
            maxchar2   = maximum(length.(string.(kwargnames)))

            str = "\n" * str * join(map(fundef[:kwargs]) do f
                str = "- `" * get_arg_item_str(f, options[:arg_types_in_desc], 
                                               options[:defaults_in_desc], maxchar, maxchar2) * 
                "`" * " : DESCRIPTION" end, "\n")
            str = str * "\n"
        end
...
function get_arg_item_str(f, show_arg_types, show_defaults, maxchar, maxchar2)
    (arg_name, arg_type, is_splat, default) = MacroTools.splitarg(f)
    str = rpad("$arg_name", maxchar2)
    if string(arg_type) != "Any" && show_arg_types
        str = str * " :: $arg_type"
    elseif show_arg_types
        str = str * " :: TYPE"
    end
    if !isnothing(default) && show_defaults
        if (default isa Expr && default.head == :call) || !(default isa Expr)
